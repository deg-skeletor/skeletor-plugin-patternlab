...
describe('When exportPatterns() is invoked', () => {
	...
	test('a pattern is exported to the root of the destination directory when the flatten option is enabled', async () => {
		const globPattern = patternLabConfig.paths.public.patterns + '*/??-components-*';
		
		const exportConfigFlatten = {
			patternGroups: [
				{
					patterns: 'components/*',
					dest: './dist/patterns/',
		...
		await patternExporter.exportPatterns(patternLabConfig, exportConfigFlatten, logger);
		expect(fse.outputFile).toHaveBeenCalledTimes(1);
		expect(fse.outputFile).toHaveBeenCalledWith(destFilepaths.test, expect.any(String));
	});
